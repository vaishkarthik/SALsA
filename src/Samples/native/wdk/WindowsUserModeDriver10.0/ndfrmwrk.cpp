...
HANDLE ghHeap = NULL;
NetworkDirect::Framework* gpFramework = NULL;


Framework::Framework() :
    m_hIocp( NULL ),
    m_hProviderChange( NULL ),
...
Framework::Init()
{
    ...
    ret = ::WSAProviderConfigChange( &m_hProviderChange, NULL, NULL );
    if( ret != 0 )
    {
        return HRESULT_FROM_WIN32( ::WSAGetLastError() );
    }
    ...
    __analysis_assume( m_hProviderChange != NULL );
    ...
    DWORD BytesRet;
    ret = ::WSAIoctl( m_Socket, SIO_ADDRESS_LIST_CHANGE, NULL, 0, NULL,
        0, &BytesRet, &m_Ov[ND_NOTIFY_ADDR_CHANGE], NULL );
    ...
    )
{
    ...
    if( pAddressList == NULL || cbRequired > *pcbAddressList )
    {
        ...
        if( ret == TRUE )
        {
            ...
            case ND_NOTIFY_PROVIDER_CHANGE:
                ...
                status = ::WSAProviderConfigChange(
                    &m_hProviderChange, &m_Ov[ND_NOTIFY_PROVIDER_CHANGE], NULL );
            ...
            case ND_NOTIFY_ADDR_CHANGE:
                ...
                status = ::WSAIoctl( m_Socket, SIO_ADDRESS_LIST_CHANGE, NULL, 0, NULL,
                    0, &bytesRet, &m_Ov[ND_NOTIFY_ADDR_CHANGE], NULL );
                ASSERT( status == 0 );
